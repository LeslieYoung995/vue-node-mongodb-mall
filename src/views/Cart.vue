              ...
              <li v-for="item in cartList">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delModalComfirm(item)">
  ...
  export default {
    data() {
      return {
        ...
        modalConfirm: false,
        delItem: {},
        // checkedAll :false 在computed中声明，无需在data中定义
      }
    },
    computed: {
      checkedAll() {    //实时改变checkAll的状态
    ...
    methods: {
      ...
      delCart() {
        axios.post("users/cartDel", {
          ...
          if (res.status == "0") {
            ...
            this.$store.commit("updateCartCount", -delCount);  //通过vuex实现删除购物车后数量变化
            this.init();
      ...
      //全选功能
      toggleCheckedAll() {
        let flag = !this.checkedAll;  //实现全选功能，computed会实时计算所以 this.checkedAll=!this.checkedAll会失效
        this.cartList.forEach((item) => {
          item.checked = flag ? '1' : '0'
        })
        axios.post('users/editAllChecked', {
          checked: flag
        }).then((response) => {
          let res = response.data;
          if (res.status == '0') {
